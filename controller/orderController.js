...
exports.updateOrderStatus = async (req, res) => {
  ...
  try {
    ...
    res.status(statusCode).json({ msg: err.message || 'Failed to update order' });
  }
};
...
exports.cancelOrder = async (req, res) => {
  ...
  try {
    const order = await Order.cancelOrderByCustomer(id, req.user.id);
    ...
    res.status(statusCode).json({ msg: err.message || 'Failed to cancel order' });
  }
};
...
exports.getOrdersByUserId = async (req, res) => {
  ...
  try {
    ...
    const result = await Order.getOrdersByUserIdAdmin(id, limit, offset);
    const totalResult = await Order.getOrdersCountByUserId(id);
